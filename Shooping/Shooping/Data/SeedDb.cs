...
public class SeedDb
{
    ...
    private async Task AddProductAsync(string name, decimal price, float stock, List<string> categories, List<string> images)
    {
        ...
        {
            Guid imageId = await _blobHelper.UploadBlobAsync($"{Environment.CurrentDirectory}\\wwwroot\\images\\products\\{image}", "products-mvc");
            //La tabla ProductImages contiene los campos  [ProductId]  y  [ImageId]
            //el ProductId lo añade automaticamente ya que lo estamos añadiendo a un prodcut.ProductImages(...
            //Le pone el id del producto donde lo estamos añadiendo al ProductId del ->prodcut.ProductImages 
    ...
    UserType userType)
    {
        ...
        if (user == null) //si no existe ese usuario con ese Email
        {
            Guid imageId = await _blobHelper
                .UploadBlobAsync($"{Environment.CurrentDirectory}\\wwwroot\\images\\users\\{image}", "users-mvc");
    ...
    private async Task CheCategoriesAsync()
    {
